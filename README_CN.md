...
### V4.34 – 模版与新手指引
- 新增个人账本“账目模版”：可新建/编辑/删除/拖拽排序，左滑样式与记录一致，记账表单可一键套用。
- 模版随备份导入导出，兼容旧版本备份文件（旧包缺少 templates 字段也能导入）。
- 模版的账户/金额/分类/用途均可留空，提示避免误覆盖；记账页无模版时提供入口跳转到设置创建。
- 首次启动界面优化：货币行与标题同一行，选择框右对齐并使用主题色。
